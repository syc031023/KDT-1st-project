$(document).ready(function() {
  ...
  // 제주도  2개 end
    ...
    let Alldata = festdata;
    console.log("revice 값 : ", receive); // data
    console.log("result 값 : ", data_result); // 
    ...
    for(let i = 0; i < Alldata.length; i++) {
      if(data_result === Alldata[i].id) {
  
        const name = Alldata[i].name;
        const address = Alldata[i].address;
        const startDate = Alldata[i].startDate;
        const endDate = Alldata[i].endDate;
        const cost = Alldata[i].cost;
        const instagram = Alldata[i].instagram;
        const describe = Alldata[i].describe;
        const detail = Alldata[i].detail;
        const category = Alldata[i].category;
        const location = Alldata[i].location;
        const contact = Alldata[i].contact;
        const poster_url = Alldata[i].poster_url;
        const img_url = Alldata[i].img_url;


        poster_url.split(':')[1]
        console.log("이미지값 :",poster_url);
        ...
        $('.fest-title-img').css('background-image', `url(${poster_url})`);
        $('.title-img').css('background-image', `url(${poster_url})`);
        $('#main-wrap').css('background-image', `url(${img_url})`);
